...
Future<void> _waitForSpeechCompletion() {
  ...
  final voices = await tts.getVoices; // غالبًا ترجع List<dynamic> من خرائط فيها name و locale
  if (voices is List) {
    // بحث بسيط عن أسماء تدل على أنثى أو ذكر
    for (var v in voices) {
      final name = (v['name'] ?? '').toString().toLowerCase();
      if (_maleVoice == null &&
          (name.contains('male') || name.contains('man') || name.contains('david') || name.contains('mark'))) {
        _maleVoice = v['name'];
      }
      if (_femaleVoice == null &&
